...
/** A value which has a one-to-one association with Bitcoin addresses in a
  * given wallet. Address handles are more efficient to deal with than the
  * actual addresses themselves, since address handles are much smaller.
  * Used in the referencing of the new HD derived addresses */
typedef uint32_t AddressHandleHD;
...
extern WalletErrors sanitiseNonVolatileStorage(uint32_t start, uint32_t end, bool quiet, int lang);
extern WalletErrors deleteWallet(uint32_t wallet_spec);
extern WalletErrors newWallet(uint32_t wallet_spec, uint8_t *name, bool use_seed, uint8_t *seed, bool make_hidden, const uint8_t *password, const unsigned int password_length, const uint8_t *transaction_pin, bool transaction_pin_used_param);
extern AddressHandle makeNewAddress(uint8_t *out_address, PointAffine *out_public_key);
extern WalletErrors getAddressAndPublicKey(uint8_t *out_address, PointAffine *out_public_key, AddressHandle ah_root, AddressHandle ah_chain, AddressHandle ah_index);
extern WalletErrors getPublicKey(uint8_t *out_address, PointAffine *out_public_key, AddressHandle ah_root, AddressHandle ah_chain, AddressHandle ah_index);
...
extern WalletErrors getPrivateKeyExtended(uint8_t *out, AddressHandle ah_root, AddressHandle ah_chain, AddressHandle ah_index);
extern WalletErrors getPublicExtendedKey(uint8_t *out, AddressHandle ah);
...
extern bool checkWalletLoaded(void);
extern bool hasTransactionPin(void);
extern uint8_t *getTransactionPINhash(void);
extern uint32_t readEntireWalletSpace(uint8_t *bulkData);
