...
//        this->factored_stage_time = this->stage_time * this->temperature_to_factor_10x(temperature) / 1000; // originally 10
        this->factored_stage_time = this->stage_time * this->temperature_to_factor_static() / 0.9; // originally 10
//        this->factored_stage_time = this->stage_time * this->temperature_to_factor_static() / 0.1; // originally 10

//        100 is too fast, no good contrast, 10 is decent contrast but too slow. 1 is great contrast but waaay too slow.
//			changing the returned value to 1, then starting over with /1
//        1/1 so-so contrast; 1/0.1 gawdawful slow but very sharp
//        Refresh sequence altered; inv>white>white>data>data. 0.1 very slow but sharp. Changing back to 1
    }
...
//        this->frame_data_repeat(new_image, EPD_normal);
    ...
    void image_sram(uint8_t *new_image)
    {
        this->frame_fixed_repeat(0xaa, EPD_compensate);
        this->frame_fixed_repeat(0xaa, EPD_white);
//        this->frame_fixed_repeat(0xaa, EPD_white);
...
//        this->frame_sram_repeat(new_image, EPD_normal);
    ...
    int temperature_to_factor_static();
